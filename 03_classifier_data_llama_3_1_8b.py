...
def answer_question(tokenizer, question, context, num_retrievals=0):
    # llama-3.1b-instruct
    messages = [
        {"role": "system", "content": f"You are a QA bot who answers the question of the user only with the information provided in the context. Context: {context}"},
...
for qa in data:
    ...
    while k < 8:
        retrieved_contexts = retrieve_contexts(question, 5, retrieved_indices) # 한 번 retrieval 시 몇 번 하는 게 좋을지도 조정 가능해야 할지..
        context = " ".join(retrieved_contexts) 
        contexts += context
        answer, num_retrievals = answer_question(tokenizer, question, contexts, num_retrievals)
        if gt.lower() in answer['content'].lower():
            break
        else:
            k += 1
    if num_retrievals > 7:
        num_retrievals = 'unanswerable'
    results.append({"question": question, "answer": answer['content'], "ground truth": gt, "retrievals": num_retrievals, "dataset": qa['dataset']})
    print(results)
    with open("qa_results_1.json", "w") as f:
        json.dump(results, f)